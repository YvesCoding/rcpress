...
要启用 AntdSite 的多语言支持，首先需要使用如下的文件结构：
...
然后，在 `.antdsite/config.js` 中提供 `locales` 选项：
...
如果一个语言没有声明 `title` 或者 `description`AntdSite 将会尝试使用配置顶层的对应值。如果每个语言都声明了 `title` 和 `description`，则顶层的这两个值可以被省略。
