...
program
  ...
  .option(
    '-p, --port <port>',
    'use specified port (default: 8080)'
  )
  .option(
    '-h, --host <host>',
    'use specified host (default: 0.0.0.0)'
  )
  .option(
    '--debug',
  ...
  .action((dir = 'docs', { host, port, debug }) => {
    wrapCommand(dev)(path.resolve(dir), {
      host,
      port,
      debug
    });
...
program
  ...
  .option(
    '-d, --dest <outDir>',
    'specify build output dir (default: .rcpress/dist)'
  )
  .option(
    '--debug',
  ...
  .action((dir = '.', { debug, dest }) => {
    ...
    wrapCommand(build)(path.resolve(dir), {
      debug,
      outDir
    });
  });
...
program
  ...
  .description(
    'copy the default theme into .rcpress/theme for customization.'
  )
  .action((dir = '.') => {
...
enhanceErrorMessages('missingArgument', argName => {
  return `Missing required argument ${chalk.yellow(
    `<${argName}>`
  )}.`;
});
...
enhanceErrorMessages(
  ...
  (option, flag) => {
    return (
      `Missing required argument for option ${chalk.yellow(
        option.flags
