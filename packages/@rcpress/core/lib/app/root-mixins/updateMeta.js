export default {
  ...
  mounted() {
    ...
    const updateMeta = () => {
      ...
      const useGlobalDescription =
        userMeta.filter(m => m.name === 'description')
          .length === 0;
      ...
      if (descriptionMetas.length) {
        descriptionMetas.forEach(m =>
          this.currentMetaTags.add(m)
        );
      }

      this.currentMetaTags = new Set(
        updateMetaTags(meta, this.currentMetaTags)
      );
    };
