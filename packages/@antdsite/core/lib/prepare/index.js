...
module.exports = async function prepare(sourceDir) {
  ...
  const overridePath = path.resolve(sourceDir, '.antdsite/override.less').replace(/[\\]+/g, '/');
  ...
  const stylePath = path.resolve(sourceDir, '.antdsite/style.less').replace(/[\\]+/g, '/');
  ...
  if (hasUserOverride && !hasUserStyle) {
    logger.tip(
      ...
      `      See: ${chalk.magenta(
        'https://antdsite.vuejs.org/default-theme-config/#simple-css-override'
      )}`
    );
  }

  // 5. handle enhanceApp.js
  // const enhanceAppPath = path.resolve(sourceDir, '.antdsite/enhanceApp.js');
