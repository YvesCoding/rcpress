...
const { dev, build, eject } = require('@antdsite/core');
const { logger } = require('@antdsite/util');

if (!semver.satisfies(process.version, requiredVersion)) {
  logger.error(
    `\n[antdsite] minimum Node version not met:` +
...
program
  ...
  .option('-d, --dest <outDir>', 'specify build output dir (default: .antdsite/dist)')
...
program
  ...
  .description('copy the default theme into .antdsite/theme for customization.')
  .action((dir = '.') => {
...
program.on('--help', () => {
  console.log();
  console.log(
    `  Run ${chalk.cyan(`antdsite <command> --help`)} for detailed usage of given command.`
  );
  console.log();
});
